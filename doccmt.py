			...
			if list_update != list_cmt:
				...
				for i in temp:
					...
					if check_dau_cach !=0:
						...
						ok_dodai = True
					if (ok_kitu == True and ok_dodai == True) and (len(save) < 100):
						doc_cmt(i)
...
form.resizable(0,0)
lb = Label(form, text= "Nhập Link Livestream",font=("san-serif",20))
lb.pack(pady=15)
link = Entry(form,font=("Conslas",20),bg = "white",width = 40)
link.pack(pady=15)
btn= Button(form, text= "SỦA",font=("san-serif",20),fg ="white", bg= "black",width =10,command = read)
btn.pack(pady=15)
form.mainloop()